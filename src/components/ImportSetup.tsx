...
export function ImportSetup({ profile, onClose }: Props) {
  ...
  return <section className="import-panel">
    ...
    {preview && <div className="import-preview">
      <div className="import-summary"><strong>Existing installation detected</strong><span>Source: {preview.sourceMinecraftVersion ? `Minecraft ${preview.sourceMinecraftVersion}` : "version not confidently detected"} · Target: Minecraft {preview.minecraftVersion} · {preview.loader === "fabric" ? "Fabric" : "Vanilla"}</span>{!preview.sameVersion && <p className="import-warning">Flint will install compatible versions for Minecraft {preview.minecraftVersion}. Original mod files will not be copied unless this is confidently a same-version setup.</p>}<div className="import-counts"><span>Ready <b>{counts.compatible}</b></span><span>Will be updated <b>{counts.resolvable}</b></span><span>Needs review <b>{counts.unknown}</b></span><span>Skipped <b>{counts.incompatible}</b></span></div><small title={preview.source}>{preview.source}</small></div>
