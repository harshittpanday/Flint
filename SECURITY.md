...
Passwords are stored by the `keyring` Windows-native backend in Windows Credential Manager. The dedicated save IPC command accepts a password once from Flint's own WebView, then never returns it to the frontend. Instance configuration contains only an opaque credential reference, exact server address, enable flag, and command templates. At launch Flint binds an ephemeral loopback-only port and passes its random token to the child process through environment variables. Flint Client can request a command only for an exact enabled server after the player explicitly types `/flintauth login` or `/flintauth register`; the launcher and client each reject repeated attempts. The password is zeroized after command construction and is never persisted in profile JSON, instance configuration, logs, Discord, diagnostics, imports, or exports.
