...
Passwords are stored by the `keyring` Windows-native backend in Windows Credential Manager. The dedicated save IPC command accepts a password once from Flint's own WebView, then never returns it to the frontend. Instance configuration contains only an opaque credential reference, exact server address, Disabled/Login/Register mode, and command templates. At launch Flint binds an ephemeral loopback-only port and passes its random token to the child process through environment variables. After a game-join reset and readiness delay, Flint Client can request a command only for the exact configured server; each request includes a fresh random connection UUID, the bridge validates it and chooses the stored mode, and the launcher and client each reject repeats within that connection while permitting a later reconnect. The password is zeroized after command construction and is never persisted in profile JSON, instance configuration, logs, Discord, diagnostics, imports, or exports. Diagnostics contain fixed lifecycle phrases only—not server addresses, usernames, passwords, or rendered commands.
