...
HUD positions are stored as normalized instance-local coordinates and edited by dragging preview elements. Zoom and Freelook are hold-to-use camera transforms; time, weather, hand, trail, and particle changes affect only the local client. Auto GG is opt-in, matches a deliberately small exact-message allowlist, and is delayed, connection-scoped, and rate-limited. Every input/render mixin delegates state to `FlintRuntime` so disabling a module restores or stops its local override.
